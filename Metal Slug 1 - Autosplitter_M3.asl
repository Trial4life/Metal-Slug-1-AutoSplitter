...
startup
{
	...
	vars.scannerTargetBossHealth = new SigScanTarget(32,
	   "00 ?? 2E 00 76 3F FF FF FF FF ?? ?? ?? ?? ?? ?? " +
	   "?? ?? 00 ?? 00 ?? ?? 00 FF FF FF FF FF FF FF FF " +
	   "?? ?? ?? 03 00 00 FF FF FF ?? FF FF 00 ?? FF ??"
	);
...
init
{
	...
	if(game.ProcessName.Equals("WinKawaks"))
	{
		...
		//The exclamation mark in the "Mission Start!" text of mission 3
...
;
		...
		//The snow during the fight against the two tanks before the boss fight
		//Starts at pixel ( 191, 218 )
	...
	else //if(game.ProcessName.Equals("mslug1"))
	{
		...
		//The exclamation mark in the "Mission Start!" text of mission 3		// #TODO: The RGB values must be adapted for Steam
		//Starts at pixel ( 167 , 114 )
		vars.colorsRunStart = new byte[]		{
													66,  97,  123, 255,
													24,  56,  74,  255,
		...
		//The snow during the fight against the two tanks before the boss fight   // #TODO: The RGB values must be adapted for Steam
		//Starts at pixel ( 191, 218 )
		vars.colorsBossStart = new byte[]		{
													99,  113, 123, 255,
													107, 138, 148, 255,
